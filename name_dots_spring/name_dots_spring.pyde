...
resting_positions = []
restore_delay = 3000  # 3 seconds in milliseconds
...
letter_tints = [color(i * 10, i * 10, i * 10) for i in range(len(main_text))]

def setup():
    size(1000, 500)  # Set the window size to 1000x500
    background(255)
    
    global font_large, font_small
    font_large = createFont("Roboto-Regular.ttf", 96)  # Load the custom TrueType font
    font_small = createFont("Roboto-Regular.ttf", 36)  # Use "Roboto-Light" from Google Fonts
...
def mouseMoved():
    ...
    for dot in dots:
        if dist(mouseX, mouseY, dot.x, dot.y) < 20:
            dot.target_x = random(0, width)  # Specify the range for random x
            dot.target_y = random(0, height)  # Specify the range for random y
